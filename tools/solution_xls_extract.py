...
def get_land_scenarios(wb, solution_category):
    ...
    for row in range(sr_tab.min_row, sr_tab.max_row):
        ...
        if col_d == "Name of Scenario:" and "TEMPLATE" not in col_e:
            ...
            has_values = (
                xls(sr_tab, row + 203, co("E")) and xls(sr_tab, row + 203, co("H")) and xls(sr_tab, row + 203, co("K"))
            )
            ...
            for addl in range(271, 285):
                if (
                    ...
                    s["avoided_deforest_with_intensification"] = convert_sr_float(
                        sr_tab.cell(row + addl, co("E")).value
                    )
            scenarios[scenario_name] = s
    return scenarios
...
def write_tam(f, wb, outputdir):
    ...
    f.write(
        f"                '{xls(tm_tab, 'V16')}', '{xls(tm_tab, 'V19')}', '{xls(tm_tab, 'V22')}', '{xls(tm_tab, 'V25')}',"
    )
    ...
    val = xls(tm_tab, "B25")
    f.write(
        f"            ['low_sd_mult', {val}, {val}, {val}, {val}, {val}, {val}, {val}, {val}, {val}, {val}, {val}],\n"
    )

    val = xls(tm_tab, "B24")
    f.write(
        f"            ['high_sd_mult', {val}, {val}, {val}, {val}, {val}, {val}, {val}, {val}, {val}, {val}, {val}]]\n"
    )
    ...
    ref_sources = extract_source_data(
        wb=wb, sheet_name="TAM Data", regions=tam_regions, outputdir=tamoutputdir, prefix="tam_"
    )
    ...
    pds_sources = extract_source_data(
        wb=wb, sheet_name="TAM Data", regions=tam_regions, outputdir=tamoutputdir, prefix="tam_pds_"
    )
        ...
        for region, cases in pds_sources.items():
            ...
            for (case, sources) in cases.items():
                if isinstance(sources, str):
                    ...
                    for (source, filename) in sources.items():
                        f.write(
                            "                      '" + source + "': THISDIR.joinpath('tam', '" + filename + "'),\n"
                        )
                    f.write("              },\n")
            f.write("            },\n")
        f.write("        }\n")
...
def normalize_source_name(sourcename):
    ...
    if "UN CES" in name and "ITU" in name and "AMPERE" in name:
        if "BASELINE" in name:
            return "Based on: CES ITU AMPERE Baseline" + suffix
        if "550" in name:
            return "Based on: CES ITU AMPERE 550" + suffix
        if "450" in name:
            return "Based on: CES ITU AMPERE 450" + suffix
        raise ValueError("Unknown UN CES ITU AMPERE source: " + sourcename)
    if "IEA" in name and "ETP" in name:
        if "2014" in name and "2DS" in name:
            return "Based on: IEA ETP 2014 2DS" + suffix
        if "2014" in name and "4DS" in name:
            return "Based on: IEA ETP 2014 4DS" + suffix
        if "2014" in name and "6DS" in name:
            return "Based on: IEA ETP 2014 6DS" + suffix
        if "2016" in name and "6DS" in name:
            return "Based on: IEA ETP 2016 6DS" + suffix
        if "ETP16" in name and "6DS" in name:
            return "Based on: IEA ETP 2016 6DS" + suffix
        if "2016" in name and "4DS" in name:
            return "Based on: IEA ETP 2016 4DS" + suffix
        if "ETP16" in name and "4DS" in name:
            return "Based on: IEA ETP 2016 4DS" + suffix
        if "2016" in name and "2DS" in name and "OPT2-PERENNIALS" in name:
            return "Based on: IEA ETP 2016 2DS with OPT2 perennials" + suffix
        if "2016" in name and "2DS" in name:
            return "Based on: IEA ETP 2016 2DS" + suffix
        if "ETP16" in name and "2DS" in name:
            return "Based on: IEA ETP 2016 2DS" + suffix
        if "2016" in name and "ANNEX" in name:
            return "Based on: IEA ETP 2016 Annex" + suffix
        if "2017" in name and "REF" in name:
            return "Based on: IEA ETP 2017 Ref Tech" + suffix
        if "2017" in name and "B2DS" in name:
            return "Based on: IEA ETP 2017 B2DS" + suffix
        if "2017" in name and "BEYOND 2DS" in name:
            return "Based on: IEA ETP 2017 B2DS" + suffix
        if "2017" in name and "2DS" in name:
            return "Based on: IEA ETP 2017 2DS" + suffix
        if "2017" in name and "4DS" in name:
            return "Based on: IEA ETP 2017 4DS" + suffix
        if "2017" in name and "6DS" in name:
            return "Based on: IEA ETP 2017 6DS" + suffix
        raise ValueError("Unknown IEA ETP source: " + sourcename)
    if "AMPERE" in name and "MESSAGE" in name:
        if "450" in name:
            return "Based on: AMPERE 2014 MESSAGE MACRO 450" + suffix
        if "550" in name:
            return "Based on: AMPERE 2014 MESSAGE MACRO 550" + suffix
        if "REF" in name:
            return "Based on: AMPERE 2014 MESSAGE MACRO Reference" + suffix
        raise ValueError("Unknown AMPERE MESSAGE-MACRO source: " + sourcename)
    if "AMPERE" in name and "IMAGE" in name:
        if "450" in name:
            return "Based on: AMPERE 2014 IMAGE TIMER 450" + suffix
        if "550" in name:
            return "Based on: AMPERE 2014 IMAGE TIMER 550" + suffix
        if "REF" in name:
            return "Based on: AMPERE 2014 IMAGE TIMER Reference" + suffix
        raise ValueError("Unknown AMPERE IMAGE-TIMER source: " + sourcename)
    if "AMPERE" in name and "GEM" in name and "E3" in name:
        if "450" in name:
            return "Based on: AMPERE 2014 GEM E3 450" + suffix
        if "550" in name:
            return "Based on: AMPERE 2014 GEM E3 550" + suffix
        if "REF" in name:
            return "Based on: AMPERE 2014 GEM E3 Reference" + suffix
    ...
    if "GREENPEACE" in name and "ENERGY" in name:
        ...
        if "ADVANCED" in name:
            return "Based on: Greenpeace 2015 Advanced Revolution" + suffix
        ...
        if "REVOLUTION" in name:
            return "Based on: Greenpeace 2015 Energy Revolution" + suffix
        if "REFERENCE" in name:
            return "Based on: Greenpeace 2015 Reference" + suffix
        raise ValueError("Unknown Greenpeace Energy source: " + sourcename)
    if "GREENPEACE" in name and "THERMAL" in name:
        if "MODERATE" in name:
            return "Based on: Greenpeace 2016 Solar Thermal Moderate" + suffix
        if "ADVANCED" in name:
            return "Based on: Greenpeace 2016 Solar Thermal Advanced" + suffix
...
def write_ad(f, wb, outputdir):
    ...
    sources = extract_source_data(
        wb=wb, sheet_name="Adoption Data", regions=ad_regions, outputdir=ad_outputdir, prefix="ad_"
    )
...
def write_custom_ad(case, f, wb, outputdir, is_land):
    ...
    scenarios, multipliers = extract_custom_adoption(
        wb=wb, outputdir=ca_dir_path, sheet_name=f"Custom {case} Adoption", prefix=f"custom_{case.lower()}_ad_"
    )
...
def write_s_curve_ad(f, wb):
    ...
    f.write(
        "        sconfig['last_percent'] = pd.Series(pd.Series(list(self.ac.pds_adoption_final_percentage.values()),\n"
    )
...
def write_ht(f, wb, has_custom_ref_ad, is_land):
    ...
    f.write(
        f"        ht_pds_adoption_final = ht_pds_adoption_final_percentage * {tam_or_tla}.loc[{final_datapoint_year}]\n"
    )
...
def write_fc(f, wb):
    ...
    f.write(
        "        self.fc = firstcost.FirstCost(ac=self.ac, pds_learning_increase_mult=" + xls(fc_tab, "C25") + ",\n"
    )
    f.write(
        "            ref_learning_increase_mult="
        + xls(fc_tab, "D25")
...
def write_oc(f, wb, is_land=False):
    ...
    if conversion_factor_fom == conversion_factor_vom:
        ...
        f.write("            conversion_factor=(" + conversion_factor_fom + ", " + conversion_factor_vom + "))\n")
    f.write("\n")
...
def extract_source_data(wb, sheet_name, regions, outputdir, prefix):
    ...
    for (region, baserow) in regions.items():
        ...
        df = pd.read_excel(
            wb, engine="openpyxl", sheet_name=sheet_name, header=baserow, index_col=0, usecols=usecols, nrows=49
        )
        df.name = region
...
def extract_custom_adoption(wb, outputdir, sheet_name, prefix):
    ...
    multipliers = {"high": xli(custom_ad_tab, "AO25"), "low": xli(custom_ad_tab, "AO26")}
    scenarios = []
    ...
    for srow in range(20, 36):
        ...
        for row in range(custom_ad_tab.min_row, custom_ad_tab.max_row):
            if normalize_source_name(xls(custom_ad_tab, row, co("B"))) == name:
                ...
                df = pd.read_excel(
                    wb, engine="openpyxl", sheet_name=sheet_name, header=row, index_col=0, usecols="A:K", nrows=49
                )
                df.rename(
                    mapper={"Middle East & Africa": "Middle East and Africa", "Asia (sans Japan)": "Asia (Sans Japan)"},
                    axis="columns",
                    inplace=True,
                )
                ...
                break
        if not skip:
            scenarios.append({"name": name, "filename": filename, "description": description})
...
def extract_custom_tla(wb, outputdir):
    ...
    df = pd.read_excel(wb, engine="openpyxl", sheet_name="TLA Data", header=644, index_col=0, usecols="B:L", nrows=49)
    df.index.name = "Year"
    df.index.astype(int)
...
def lookup_unit(tab, row, col):
    unit_mapping = {
        "Million hectare": "Mha",
        "MMt FlyAsh Cement (Sol) or MMt OPC (Conv) (Transient)": "MMt",
        "Billion USD": "US$B",
        "million m2 commercial floor space": "Mm\u00B2",
        "Million Households": "MHholds",
        "Million m2 of Comm.+Resid. Floor Area Equiv. for Cold Climates": "Mm\u00B2",
        "Giga-Liter Water": "GL H\u2082O",
        "Million Metric tonnes per year": "MMt",
        "million tonne-km": "Mt-km",
        "million tonne-kms": "Mt-km",
        "Residential and Commercial roof area, m2": "m\u00B2",
        "Residential and Commercial roof area,  m2": "m\u00B2",
    }
...
def output_solution_python_file(outputdir, xl_filename):
    ...
    if "BIOSEQ" in xl_filename or "PDLAND" in xl_filename or "L-Use" in xl_filename or "AEZ Data" in wb.sheetnames:
        is_rrs = False
        is_land = True
    ...
    f.write("scenarios = ac.load_scenarios_from_json(" "directory=THISDIR.joinpath('ac'), vmas=VMAs)\n")
    f.write("\n\n")
    ...
    if warn_counts["unknown_formula"] > 0:
        warnings.warn(
            f"Extraction encountered {warn_counts['unknown_formula']} unknown formulas in values on ScenarioRecord tab"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--outputdir",
        default=None,
        help="Directory to write generated code and files to.  Defaults to the same directory as excelfile.",
    )
